...
$jumlahData = count(query("SELECT * FROM posts where category_id = '$id'"));
$jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
$halamanAktif = (isset($_GET["halaman"])) ? $_GET["halaman"] : 1;

$awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;

$halaman = mysqli_query($conn, "SELECT *, posts.created_at FROM posts inner join categorys on posts.category_id = categorys.id where category_id = '$id' limit $awalData, $jumlahDataPerhalaman");
...
$array = mysqli_fetch_array($halaman);
...
<head>
    ...
    <title><?php echo ($array) ? $array['name_categ'] . ' -' : ''; ?> SyahriProject.id</title>
    ...
    <div class="container-fluid sidebar-fixed">
        <div class="row ">
            ...
            <div class="col-md-6">
                <main>
                    <div class="container">
                        <h1 class="mt-2">Category : <?php echo $_GET['name']; ?></h1>
                        <?php if (!$array) : ?>
                        ...
                        <ul class="pagination justify-content-center">
                            <?php if (isset($_POST["cari"]) < $halaman) : ?>
                                <?php if ($halamanAktif > 1) : ?>
                                    <li class="page-item">
                                        <a class="page-link" href="?categ=<?php echo $_GET['categ']; ?>&&name=<?php echo $_GET['name']; ?>&&halaman=<?= $halamanAktif - 1; ?>">Previous</a>
                                    </li>
                                <?php endif; ?>
                                <?php for ($i = 1; $i <= $jumlahHalaman; $i++) : ?>
                                    <?php if ($i == $halamanAktif) : ?>
                                        <li class="page-item"><a style="color: red; font-wight: bold;" class="page-link" href="?categ=<?php echo $_GET['categ']; ?>&&name=<?php echo $_GET['name']; ?>&&halaman=<?= $i; ?>"><?= $i; ?></a></li>
                                    <?php else : ?>
                                        <li class="page-item"><a class="page-link" href="?categ=<?php echo $_GET['categ']; ?>&&name=<?php echo $_GET['name']; ?>&&halaman=<?= $i; ?>"><?= $i; ?></a></li>
                                    <?php endif; ?>
                                <?php endfor; ?>
                                <?php if ($halamanAktif < $jumlahHalaman) : ?>
                                    <li class="page-item ">
                                        <a class="page-link" href="?categ=<?php echo $_GET['categ']; ?>&&name=<?php echo $_GET['name']; ?>&&halaman=<?= $halamanAktif + 1; ?>">Next</a>
                                    </li>
                                <?php endif; ?>
                            <?php endif; ?>
                        </ul>
