...
$jumlahData = count(query("SELECT * FROM posts"));
$jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
$halamanAktif = (isset($_GET["halaman"])) ? $_GET["halaman"] : 1;

$awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;

$halaman = mysqli_query($conn, "SELECT * FROM posts ORDER BY created_at DESC LIMIT $awalData, $jumlahDataPerhalaman");
            ...
            <ul class="pagination justify-content-center">
              <?php if (isset($_POST["cari"]) < $halaman) : ?>
                <?php if ($halamanAktif > 1) : ?>
                  <li class="page-item">
                    <a style="color: black;" class="page-link" href="?halaman=<?= $halamanAktif - 1; ?>">Previous</a>
                  </li>
                <?php endif; ?>
                <?php for ($i = 1; $i <= $jumlahHalaman; $i++) : ?>
                  <?php if ($i == $halamanAktif) : ?>
                    <li class="page-item"><a style="font-wight: bold;" class="page-link" href="?halaman=<?= $i; ?>"><?= $i; ?></a></li>
                  <?php else : ?>
                    <li class="page-item"><a style="color: black;" class="page-link" href="?halaman=<?= $i; ?>"><?= $i; ?></a></li>
                  <?php endif; ?>
                <?php endfor; ?>
                <?php if ($halamanAktif < $jumlahHalaman) : ?>
                  <li class="page-item ">
                    <a style="color: black;" class="page-link" href="?halaman=<?= $halamanAktif + 1; ?>">Next</a>
                  </li>
                <?php endif; ?>
              <?php endif; ?>
            </ul>
